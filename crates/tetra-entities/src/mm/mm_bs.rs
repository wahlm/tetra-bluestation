...
use tetra_core::{BitBuffer, Sap, SsiType, TetraAddress, TdmaTime, assert_warn, unimplemented_log};
...
impl MmBs {
    ...
    fn emit_brew_update(
        ...
        let update = BrewSubscriberUpdate {
            issi,
            groups,
            action,
        };
    ...
    fn rx_u_attach_detach_group_identity(&mut self, queue: &mut MessageQueue, mut message: SapMsg) {
        ...
        let accepted_gid= self.try_attach_detach_groups(
            queue,
            message.dltime,
            issi,
            &pdu.group_identity_uplink.unwrap(),
        );
